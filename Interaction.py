...
class BoundingBox(Interaction):
    ...
    def apply(self):
        ...
        close_nodes = [node for node in nodes if node.p.x < x_max and node.p.x > x_min and node.p.y < y_max and node.p.y > y_min]
    ...
    def draw(self):
        pygame.draw.lines(screen, (100,100,100), closed = True,
            points = [world_to_screen_transform(self.a),
                      world_to_screen_transform(self.b),
                      world_to_screen_transform(self.c),
                      world_to_screen_transform(self.d)],
            width = 2)
